...
\todo{Add brillouin area figure}
In this analysis, a different approach based on the 6T5 condition, which is required for each selected event in this analysis, is used for the exposure calculation. For this calculation, the 6T5 hexagon is taken to be the smallest possible detection unit for the $\nu$ event. The effective area i.e. the area seen by the incident cosmic neutrino for this detection unit at full efficiency is given by the Brillouin area~\cite{PierreAuger:2010zof}, $A_{6T5} = 1.95km^2$ as shown by the shaded area in fig~\ref{}. The aperture for this detector unit is dependent on the energy of the primary neutrino ,the slant depth in the atmosphere of the first inteeraction, X, neutrino flavor (crossection), type of the interaction (CC or NC), zenith angle, azimuth angle and the point of impact of the shower on the ground. The effective \textit{acceptance} of the detector unit can be written as follows:
...
\todo{Add the plot for effective mass}
...
This section aims to detail the systematic uncertainties which can arise during the course analysis and their contribution to the exposure. The systematic uncertainities in an analysis can arise at any point. The investigations starts already at the simulation level where the uncertainties from the primary interaction, hadronic interaction models, thinning etc. can lead to uncertainties in the neutrino detection efficiency. At the next step the uncertainities in the reconstruction algorithms could also affect the final exposure. The crossections used for the final calculation of exposure could also prove to be a source of systematic uncertainities. Other parameters such as the number of hexagons are very well defined parameters and are not expected to have a contribution to the systematic uncertainities in the exposure. For this analysis the systematic uncertainities are either sourced through previous analysis~\cite{gap_systematics} or are calculated for certain cases. Since the analysis has been done once before albeit without the new triggers the sources of systematic uncertainties though not exactly the same are very similar to~\cite{gap_note_2013}. 
The sources of systematic uncertainities considered for this analysis are discussed below:
\begin{description}
  \item \textbf{Primary interaction generator} For the simulations used in this analysis which is based on CORSIKA 7.2 the only available option for the primary interaction generator is the HERWIG~\cite{} code. Thus, it was not possible to compare the differences that could arise due to the use of different primary interaction generators. These systematic uncertainities were taken from ~\cite{gap_systematics} where PYTHIA~\cite{} and several versions of HERWIG are compared for CORSIKA 6. The uncertainties were found to be [+2.6\% , 3.7\%]. 
  \item \textbf{Parton Distribution function} Again the uncertainities are taken from ~\cite{gap_systematics}since this functionality is again not available in CORSIKA 7. The systematic uncertainties on exposure quoted in the source are [+4\% , -5\%].
  ...
  \item \textbf{Shower simulator and thinning algorithm} The systematic uncertainities due to the differing shower simulation programs was estimated thorough the comparison quoted in~\cite{Huege:2022xbo} for both hadronic cascades and electromagnetic showers. The differences seen in different softwares are of the order $\sim 15\%$ which agree nicely with the results from~\cite{gap_systematics}. The uncertainities due to thinning were also calculated in~\cite{gap_systematics} and were found to be $\sim 7\%$.
  ...
  \item \textbf{Neutrino cross-sections} The systematic uncertainities due to the neutrino cross-sections were taken from~\cite{Cooper-Sarkar:2011jtt} which are of the order $\pm 7\%$.
\end{description}

Table ~\ref{tab:systematic_uncertainties} summarises the  source of systematic uncertainities and their contribution to the total exposure. The total systematic uncertainities are calculated by adding them in quadrature. As discussed later in sec.~\ref{subsec:Conrad} the uncertainities are directly incorporated in the limit by applying the Conrad method~\cite{Conrad:2002kn} to calculate the confidence intervals. 

\begin{table}[h!]
  \centering
  \begin{tabular}{ |P{4.0cm}||P{4.0cm}| }
    \hline
    Source of systematic uncertainities & Contribution to the exposure \\
    \hline
    Primary interaction generator & +3\% \enspace -4\%  \\
    Parton Distribution function & +4\% \enspace -5\%  \\
    Hadronic interaction models &\null \enspace -5\% \\ 
    Shower simulator &  +15\% \enspace \null   \\
    ...
    \hline
    Total uncertainities & +20\% \enspace -14\% \\
    \hline
  \end{tabular}
...
\begin{figure}[t!]
  ...
  \caption{Combination of Integral and differential upper limits for a UHE$\nu_s$ diffuse flux in the DG$\mathrm{_{low}}$ for the time period 2004-2021. The limits for the time pweriod 2004-2013 are plotted in orange and the limits for the time period 2014-2021 are plotted in purple. The combined limits are plotted in black.}
