...
% \begin{figure}[t!]
%   \centering
%   \subcaptionbox{SChematic of plane-front approximation  \label{sub:fig:plane_fit}}{\includegraphics[width=.45\linewidth]{thesis_figures/Nu_analysis/Plane_fit.pdf}}
%   \hfill
...
\begin{figure}
  \centering
  \subcaptionbox{CC.\label{sub:fig:FD_site_drawing}}{\includegraphics[width=.48\linewidth]{thesis_figures/Nu_analysis/MC_control/Theta_rec_CC_wnt_redcut.pdf}}
  \hfill
  \subcaptionbox{NC\label{sub:fig:FD_schematic}}{\includegraphics[width=.48\linewidth]{thesis_figures/Nu_analysis/MC_control/Theta_rec_NC_wnt_redcut.pdf}}
  \caption{Reconstructed Theta}
  \label{fig:Rec_Theta}
\end{figure}
...
The inclined shower selection removes $\sim x\%$ of showers. These showers were previously discussed in section~\ref{sec:reco_possel}. Most of these events are showers which interact very close to the ground which can lead to a mis-reconstructed zenith angle. Further, calculating the impact of $\Delta \theta_{rec}$ a stronger both CC and NC events observe a strong reduction. Comparing to the analysis with old triggers this decrease is $\sim$10\% higher for CC but similar for the one observed for NC interactions. The error distributions for both channels are given in fig~\ref{}. The NC error seems to worsen with increasing zenith angle and reduction of shower energy. These are the two areas where the reconstruction algorithm does not perform properly as in both cases the number of triggered stations are very few and additionally in the second case certain geometrical patterns such as a line can impact the zenith angle determination. For the CC the main culprit for the higher decrease with the inclusion of new triggers is attributed to the overall worsening of the angular reconstruction with the inclusion of new trigger. This can be due to multiple reasons such as lack of a tuned segment selection algorithm for the new triggers, the new triggers being more sensitive to noise etc. Further work which could not be implemented in this thesis is envisioned for the future to improve the angular reconstruction algorithm and some ideas regarding this are presented in the Outlook section of the thesis.
