...
\todo{Add comparison picture of plane and spherical fit EVNTBrowser event}
...
\begin{figure}[h!]
  ...
  \hfill
  \subcaptionbox{$\nu_e$ CC simulations.\label{sub:fig:Rec_theta_NC_comp}}{\includegraphics[width=.48\linewidth]{thesis_figures/Nu_analysis/MC_control/Theta_rec_NC_comp_redcut.pdf}}
...
\begin{figure}[h!]
  \centering
  \subcaptionbox{$\mathrm{\theta_{Rec} - \theta_{MC}}$ comparison.\label{sub:fig:Res_theta_CC_comp}}{\includegraphics[width=.48\linewidth]{thesis_figures/Nu_analysis/MC_control/Theta_res_CC_comp_wnt_redcut.pdf}}
  \hfill
...
\begin{figure}[h!]
  ...
  \caption{(a,b,c)The residual dependence on energy, zenith angle and slant depth for all reconstructed simulations. The errorbars are not the errors on the mean but signify the spread of the distribution. (d) The AoP distribution for the T5 stations for the sample with new triggers (open blue histogram) and old triggers (filled red histogram) is also shown.}
  \label{fig:Res_comp}
\end{figure}
...
\todo{Maybe check with just CC sample and see if the same conclusion is reached}
  ...
  \centering
   ...
   \begin{subfigure}[r]{.48\textwidth}
    ...
    \begin{subfigure}[r]{.48\textwidth}
      ...
      \caption{$ 70.5^{\circ} <\theta_{\text{Rec}} \leq 76.5^{\circ}$}
   \end{subfigure}
   \caption{Fisher Cut determination for the different sub-angular regions. The $\mathcal{F}$ distributions of the background training sample are plotted in purple. The exponential fit is shown in black with green marking the fitting region for the fit. The numbers in text indicate the comaparison for Observed and predicted events in each region, see text for more details. The red line indicates the $\mathcal{F}_{cut}$ obtained from the fit.}
...
\todo{Add figure for AoP modulation CC and NC and zenith angles vs Slant depth }
